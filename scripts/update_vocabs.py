...
def add_vocabs(vocabs: List[Path], mappings: dict):
    for vocab in vocabs:
        r = httpx.post(
            "http://fuseki.surroundaustralia.com/gsq-vocabs/data",
...
def remove_vocabs(vocabs: List[Path], mappings: dict):
    for vocab in vocabs:
        r = httpx.post(
            "http://fuseki.surroundaustralia.com/gsq-vocabs/update",
...
def get_graph_uri_for_vocab(vocab: Path) -> URIRef:
    ...
    have one and only one ConceptScheme per file and that the gsq VocPrez installation uses the ConceptScheme URI
    as the Graph URI"""
...
if __name__ == "__main__":
    ...
    r = httpx.get("http://gsq.surroundaustralia.com/cache-reload")
